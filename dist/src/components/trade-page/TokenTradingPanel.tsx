...
const TokenTradingPanel: React.FC<TokenTradingPanelProps> = ({
  ...
  // const fetchOHLCData = async (): Promise<CandlestickData[]> => {
  ...
  //       return [];
  //     return response.data.map((item: OHLCData) => ({
    ...
    >
      ...
      {!isCurveCompleted && (
        <div className="flex items-center justify-between mb-4">
          ...
          />
          {/* <motion.button className="text-sm text-purple-400 flex items-center space-x-1">
            <Settings size={14} />
